...
FROM (
    ...
    SELECT 2, 'Number', 'ACTIVE_JOBS',
        CAST(SUM(CASE WHEN status IN ('Pending', 'Queued', 'Running') THEN 1 ELSE 0 END) AS DOUBLE)
    FROM jobs WHERE job_type = ?
    ...
    SELECT 2, 'Percentage', 'KILL_RATE',
        CAST(ROUND(100.0 * SUM(CASE WHEN status = 'Killed' THEN 1 ELSE 0 END) / NULLIF(COUNT(*), 0), 2) AS DOUBLE)
    FROM jobs WHERE job_type = ?
